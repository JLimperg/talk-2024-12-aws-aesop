...
\begin{frame}
  \frametitle{Overview: Tree Search}

  Aesop is a proof search tactic for Lean 4.

  It is based on tree search, similar to Coq's and Isabelle's \texttt{auto}.

  Proof rules are arbitrary user-specified tactics.

  % TODO example
\end{frame}

\begin{frame}
  \frametitle{Overview: Simplifier}

  Aesop integrates Lean's simplifier as a built-in rule.

  % TODO example
\end{frame}

\begin{frame}
  \frametitle{Overview: Built-In Rules}

  Aesop provides built-in rules for basic logical reasoning, case splitting, \dots

  % TODO example
\end{frame}

\begin{frame}
  \frametitle{Overview: Best-First Search with Metavariables}

  Aesop supports best-first search with metavariables.

  % TODO example
\end{frame}

\begin{frame}
  \frametitle{Overview: Proof Script Generation}

  Aesop can generate a tactic script for the proof it found.

  % TODO example
\end{frame}

\begin{frame}
  \frametitle{Overview: Forward Reasoning}

  Aesop will soon have efficient forward rules.
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    ...
    \item Tree search backend for LLMs: \fullcite{song2023} % TODO cite ABEL
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{rapp}
    \node {$Γ,\, n~m : ℕ,\, h : 0 < \mathrm{min}~n~m  ⊢ T$} % TODO array
      child {node {$Γ,\, n : ℕ,\, h : 0 < \mathrm{min}~n~m,\, \orange{h' : 0 < n} ⊢ T$}};
  \end{rapp}
\end{frame}
